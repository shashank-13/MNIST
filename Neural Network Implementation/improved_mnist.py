...
class Contain():
    ...
    def stochasticGradient(self, training_data, epochs, eta, incrementer,labda, test_data=None):
        if test_data:
        ...
        for i in xrange(epochs):
            ...
            for p in temp_training:
                self.update_function(p, eta,labda,n)
    ...
    def update_function(self, train, eta,labda,n):
        ...
        self.weights = [(1 - eta * (labda / n)) * w - (eta / len(train)) * nw
                        for w, nw in zip(self.weights, temp_weight)]
        self.bias = [b - (eta / len(train)) * nb
                       for b, nb in zip(self.bias, temp_bias)]
...
obj = Contain([784, 100, 10])
training_data, validation_data, test_data = obj.load_data_wrapper()
print "Parameters Used : \n Eta = 3.0 ,Lamda=36 ,Hidden Layers = 100,Batch size=10 "
obj.stochasticGradient(training_data,30,3.0,10,36, test_data=test_data)
